...
The full details of this are in the word document and SPSS data and syntax files withi the directory "C:\Users\Documents\MECSH\By Site\r@h\Risk Factors\Mother vulnerablity data"
...
Once data was cleaned to remove particiants who had too complex a case record to work within this analysis, and records without dates (or dates which occured too early). There were `r D.Rows_df_srf` records for `r E.Rows_Px_srf` participants remaining
...
For each disclosure (row in table `var_rcd`) determine
...
#### worked example: Domestic Violence
...
In total, `r F.disc_rows` records were returned for `r topic` (`r df_srf[Vars,10]`).    
...
```{r, include=FALSE}
  ...
  get_site_data <- function(DV_rcd_df,row_num) {
    ...
    before_df <- SPSS_HV %>% filter(ResearchID == Px_id & date_of_visit < Date_Dsc) %>% arrange(desc(date_of_visit))
    if(nrow(before_df) > 0) before_df$PreHV <- seq.int(nrow(before_df))
    Px_Bf_cnt <- nrow(before_df)
    # get data at disclosure
    at_df <- SPSS_HV %>% filter(ResearchID == Px_id & date_of_visit == Date_Dsc) %>% arrange(desc(date_of_visit))
...
fetched_data <- fetched_data[c(72,9,10,1:8,11:71)]
...
Pre_disc_act <- Pre_disc_act %>% mutate(dups = duplicated(Pre_disc_act[,2:72])) %>% filter(dups==FALSE)
...
merged <- SPSS_HV %>% left_join(Pre_disc_act[c(1,2,4)], by = "Table_ID")
...
2x2 contingency table for `r topic`.

```{r, include=FALSE}
#pre data table
temp <- merged %>% select(Bi_Categ,inf_well_1:env_res_5,planning_1:tools_9)

eg <- temp %>% group_by(Bi_Categ, planning_4) %>% summarise(n=n()) %>% spread(planning_4,n)
