...
params:
   Vars: !r c(4,8)
   topic: !r "Domestic Violence"
---

## MECSH r@h Disclosures

Set up R environment and load data home visit data (SPSS_r@h - v10), and disclosure data (srf 050917 v10)
...
The full details of this are in the word document and SPSS data and syntax files within the directory 'C:/Users/Documents/MECSH/By Site/r@h/Risk Factors/Mother vulnerablity data'.  In short, data was included if the column 'IncludeData' equaled 'Yes'.
...
Once data was cleaned to remove particiants (2) with case records too complex to work within this analysis, and records without dates (or dates which occured too early). There were `r D.Rows_df_srf` records for `r E.Rows_Px_srf` participants remaining

#### Distribution of risk disclosure

```{r, echo=FALSE}
ggplot(Px_risks, aes(x=Yes, y=n)) + geom_area(fill = "sky blue") + ggtitle("Disclosures per mother") + xlab("number of disclosures") + ylab("number of mothers")
...
The following date was disclosed my mothers or parents to the nurse home visitors.   
...
##### Cleaning SPSS_v10 HV table

```{r, include=FALSE}
v10_summ <- SPSS_v10 %>% select(Valid_Check_new, Valid_new) %>% group_by(Valid_Check_new, Valid_new) %>% summarise(n=n()) %>% spread(Valid_new, n)
#v10_summ <- rbind(v10_summ, data.frame(Valid_Check_new="Total",t(colSums(v10_summ[,-1]))))
SPSS_HV <- SPSS_v10 %>% filter(as.numeric(Valid_Check_new) %in% c(1,6,7,8,9,10))
Risk_tbl
  # trim SPSS HV file
  SPSS_HV <- SPSS_HV[c(1:10, 215:231, 251, 255:297)]
  sum(duplicated(SPSS_HV$Table_ID))
```

In the Full SPSS data set from MCRI there are `r nrow(SPSS_v10)` rows of data.   

`r kable(v10_summ)`  

After removing the records falgged as A.Missing, B.No Visit content, c.Low act, Group session and D.Failure to attend. There are `r nrow(SPSS_HV)` rows which will be considered in this analysis.  
...
Determine if the activities which occured in the HV's just prior to disclosure, or at disclosure' were different from the activities which occured at other times in the program.

For each disclosure determine  
 a. which rows in the HV table happened at disclosure, and  
 b. which row/s happened before
 
 Record the number the rows returned, based on their chronological date
...
B. For each disclosure determine  
 i. which rows in the HV table happened before  
 ii. which row/s happened at the event

C. return HV records and incremental counts for both searches (i and ii)

D. bind two sets of records, 

E. delete the records which occur twice, keeping the 'At' record where possible.

F. join this data to main HV table

G. analyse odds of activity occuring in the 3 seperate categories (Normal, At or Before)
...
Distribution of disclosures `r kable(Px_var_dist)`

The average age of the child at disclosure was `r age_disc`.  

```{r, echo=FALSE}

ggplot(var_rcd, aes(Age_at_Disclosure)) + geom_histogram(binwidth = 6, fill = "sky blue", color = "DeepSkyBlue") + labs(x="Age at Disclosure (weeks)", title = "Distribution of disclosure by child age")
...
var_rcd$Px_Bf_cnt <- Px_Bf_cnt
var_rcd$Px_At_cnt <- Px_At_cnt
...
In total, `r H.All_Rows` HV's occured on the day or prior to these disclosures.   
This includes `r colSums(Px_At_cnt)` on the day of disclosure, and `r colSums(Px_Bf_cnt)` before.  

The average number of Home visits priot to disclosure was `r round(mean(HVs_disc$max), digits=3)`.  

```{r, echo=FALSE}

ggplot(HVs_disc, aes(max)) + geom_histogram(binwidth = 2, fill = "sky blue", color = "DeepSkyBlue") + labs(x="HVs at Disclosure", title = "Distribution of disclosure by number of preceeding HVs") 

```

##### E. trim records and remove duplicates.
```{r, include=FALSE}
...
After trimming the HV records to include only visits that occured on the day of disclosure, or were one one of (up to) three vists which occured prior, there are `r  I.Rows_pre_disc` HV records. This includes `r J.Rows_At` on the day of disclosure, and `r K.Rows_Bf` before.  
After removing the HV's that occur twice, there are `r M.Rows_pre_disc` HVs to analyse.  This includes `r N.Rows_At` on the day of disclosure, and `r O.Rows_Bf` before.  
...
This table shows the number of partipcants with HV's, and the total number of HV's in each of the 3 categories.   
`r kable(HV_type)`   

NOTE: HV's should add to `r nrow(SPSS_HV)`, Participants columns will add to more than the total number of total number of partipcants as many will have HV's in mulitple categories.
...
The Data table developed above has three categories for Home Visits. For this round of analyses the Home Visits which occured At and Before the disclosure have been combined into 1 group.  All Home Visits are therefore categorised as either, associated with disclosure (Ass) or not associated with disclosure (normal).

2x2 contingency table for `r params$topic` and activity mat_well_4.
...
This table has been used to generate the percentage of 'Yes' responses in the associated group (e.g. Yes/(Yes+No)*100 = `r perc_yes_ass`), and the percentage of Yes's in the Normal or not associated group = `r perc_yes_norm`.  
