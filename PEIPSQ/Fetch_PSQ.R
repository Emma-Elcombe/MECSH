...
rows_fetched <- max(row(PSQ_sites))
...
#STEP 1
try(rm(PSQ_sites))
PSQ_sites <- subset(sites, sites$survey == "PSQ" & sites$sub_site != "Base" & sites$site == "Jersey") 
curr_site <- PSQ_sites$site[1]
rows_fetched <- max(row(PSQ_sites))
...
for (i in seq_len(nrow(PSQ_sites))) {
  if (exists("sites_data")) {
    sites_data <- rbind(sites_data, get_site_data(PSQ_sites,i))
    flog.trace(" - rows added to sites_data", name='logger.c')
  } else {
    sites_data <- get_site_data(PSQ_sites,i)
    flog.trace(" - rows added to sites_data", name='logger.c')
  }
} 
...
#STEP 1
try(rm(PSQ_sites))
PSQ_sites <- subset(sites, sites$survey == "PSQ" & sites$sub_site != "Base" & sites$site == "SydneyLHD") 
curr_site <- PSQ_sites$site[1]
rows_fetched <- max(row(PSQ_sites))
flog.info("%s %s sites selected", rows_fetched, curr_site, name='logger.c')
...
#STEP 1
try(rm(PSQ_sites))
PSQ_sites <- subset(sites, sites$survey == "PSQ" & sites$sub_site != "Base" & sites$site == "Lewisham") 
curr_site <- PSQ_sites$site[1]
rows_fetched <- max(row(PSQ_sites))
flog.info("%s %s sites selected", rows_fetched, curr_site, name='logger.c')
...
#STEP 1
try(rm(PSQ_sites))
PSQ_sites <- subset(sites, sites$survey == "PSQ" & sites$sub_site != "Base" & sites$site == "Plymouth") 
curr_site <- PSQ_sites$site[1]
rows_fetched <- max(row(PSQ_sites))
flog.info("%s %s sites selected", rows_fetched, curr_site, name='logger.c')
...
#STEP 1
try(rm(PSQ_sites))
PSQ_sites <- subset(sites, sites$survey == "PSQ" & sites$sub_site != "Base" & sites$site == "Somerset") 
curr_site <- PSQ_sites$site[1]
rows_fetched <- max(row(PSQ_sites))
flog.info("%s %s sites selected", rows_fetched, curr_site, name='logger.c')
...
#STEP 1
try(rm(PSQ_sites))
PSQ_sites <- subset(sites, sites$survey == "PSQ" & sites$sub_site != "Base" & sites$site == "RH_Victoria") 
curr_site <- PSQ_sites$site[1]
rows_fetched <- max(row(PSQ_sites))
flog.info("%s %s sites selected", rows_fetched, curr_site, name='logger.c')
...
# Merge data files
merged <- rbind(PSQ_base, PSQ_1jer_load_file, PSQ_2syd_load_file,
      PSQ_3lew_load_file, PSQ_4ply_load_file, PSQ_5som_load_file, PSQ_6vic_load_file)
